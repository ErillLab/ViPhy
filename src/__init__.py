...
def go():
    ...
    analysis_type = u_input.get_analysis_type()
    ...
    for file in os.listdir("../" + input_folder):  # Navigates into the input_folder
        n_files += 1
        error_list = preprocess_phase.preprocessing_phase(file, input_folder, analysis_type, working_folder)
    ...
    if len(error_list) < n_files - 1:
        alignment = Blast()
        # Builds a database
        distance_dictionary, coverage_vector_dictionary = alignment.make_blast_database(working_folder, e_value)
